...
| `gold_price` | `xauusd_spot` | `historical_price_series` | Accepted Gold release source is Coinbase `XAU-USD` spot by date | `backtester/importers/gold/import_gold_daily.js` | pending rebuild | pending rebuild | pending rebuild | `2024-12-31` | `IN_PROGRESS` | `warehouse refresh pending` | Legacy `gold_spot_usd` rows are GLD proxy only and are not accepted for Gold release. |
