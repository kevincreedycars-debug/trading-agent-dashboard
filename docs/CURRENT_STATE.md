...
- Timestamped Gold evaluation, input lineage audits, chronological diagnostics and local reports are consolidated. The confirmed unordered 25-row Gold history query remains a production input defect. The prepared repair is unapplied and requires isolated n8n validation. An offline package now checks its exact exported normalization consumer against 600 synthetic rows; see `GOLD_HISTORY_PATCH_VALIDATION.md`.
