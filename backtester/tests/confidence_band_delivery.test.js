...
test("dashboard wording prefers Historical calls and does not feed historical results back into live call generation", () => {
  ...
  assert.ok(scriptSource.includes("Current Live Calls and Historical Accuracy"));
  assert.ok(scriptSource.includes("Current Live Layer 1 Calls — Historical Accuracy"));
  assert.ok(scriptSource.includes("Current Live Layer 2 Calls — Historical Accuracy"));
  assert.ok(scriptSource.includes("Historical comparison used"));
  assert.ok(scriptSource.includes("LIVE CALL"));
  assert.ok(scriptSource.includes("LIVE TRADE"));
  assert.ok(scriptSource.includes("LIVE STATE"));
  assert.ok(scriptSource.includes("EXACT MATCH"));
  assert.ok(scriptSource.includes("FALLBACK"));
  assert.ok(scriptSource.includes("Exact match: same market, direction and confidence band"));
  assert.ok(scriptSource.includes("Fallback: same market and confidence band, both directions"));
  assert.ok(scriptSource.includes("Fallback: pooled Layer 1, same direction and confidence band"));
  assert.ok(scriptSource.includes("Fallback: pooled Layer 1 confidence band"));
  assert.ok(scriptSource.includes("Fallback: pooled Layer 2, same direction and confidence band"));
  assert.ok(scriptSource.includes("Fallback: pooled Layer 2 confidence band"));
  assert.ok(scriptSource.includes("Current live state: NO CLEAR BIAS. No directional historical accuracy is shown because there is no active directional call."));
  assert.ok(scriptSource.includes("Current live state: NO TRADE. No directional historical accuracy is shown because there is no active BUY or SELL call."));
