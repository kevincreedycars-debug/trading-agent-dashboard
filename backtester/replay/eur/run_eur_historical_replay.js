...
const {
  buildReplayOutput,
  parseLogicVersion
} = require("./eur_replay_core");

const REPLAY_VERSION = "eur_historical_replay_v1";
const SOURCE_WORKFLOW = "eur_historical_replay";
...
async function fetchExistingObservation(supabaseUrl, serviceRoleKey, sourceSnapshotId) {
  ...
  url.searchParams.set("agent_name", "eq.EUR");
...
async function upsertObservation(supabaseUrl, serviceRoleKey, snapshot) {
  const sourceSnapshotId = snapshot.id || `EUR|${snapshot.snapshot_date}`;
  ...
  const payload = {
    ...
    market_regime: {
      ...
      ecb_bias: snapshot.ecb_bias || null,
      global_growth_regime: snapshot.global_growth_regime || null,
      ez_stress_flag: snapshot.ez_stress_flag || null
    },
    ...
    data_quality: {
      ...
      pmi_coverage_status: snapshot.pmi_coverage_status
    }
  };
...
async function upsertPrediction(supabaseUrl, serviceRoleKey, observationId, verdictId, prediction) {
  const payload = {
    ...
    mapping_notes: "Historical EUR replay writes canonical following-24hrs predictions from the parity-validated 24h replay core.",
  ...
  for (const snapshot of selectedSnapshots) {
    ...
    await upsertSingletonByObservation(
      ...
      observation.id,
      {
        ...
        prompt_version: null,
        weight_model_version: "eur_weighted_logic_live_export_v1",
        conviction_model_version: "eur_conviction_logic_live_export_v1",
        workflow_version_id: REPLAY_VERSION,
        repo_commit_sha: null,
        notes: "Historical EUR replay generated inside /backtester from the parity-validated 24h replay core."
      }
    );
...
if (require.main === module) {
  run().catch((error) => {
    console.error("EUR historical replay failed.");
