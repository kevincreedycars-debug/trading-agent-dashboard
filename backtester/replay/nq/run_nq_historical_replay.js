...
const {
  buildReplayOutput,
  parseLogicVersion
} = require("./nq_replay_core");

const REPLAY_VERSION = "nq_historical_replay_v1";
const SOURCE_WORKFLOW = "nq_historical_replay";
...
async function fetchExistingObservation(supabaseUrl, serviceRoleKey, sourceSnapshotId) {
  ...
  url.searchParams.set("agent_name", "eq.NQ");
...
async function upsertObservation(supabaseUrl, serviceRoleKey, snapshot) {
  const sourceSnapshotId = snapshot.id || `NQ|${snapshot.snapshot_date}`;
...
async function upsertPrediction(supabaseUrl, serviceRoleKey, observationId, verdictId, prediction) {
  const payload = {
    ...
    mapping_notes: "Historical NQ replay writes canonical predictions from the parity-validated live deterministic export logic.",
  ...
  for (const snapshot of selectedSnapshots) {
    ...
    await upsertSingletonByObservation(
      ...
      observation.id,
      {
        ...
        prompt_version: null,
        weight_model_version: "nq_weighted_logic_live_export_v1",
        conviction_model_version: "nq_conviction_logic_live_export_v1",
        workflow_version_id: REPLAY_VERSION,
        repo_commit_sha: null,
        notes: "Historical NQ replay generated inside /backtester from the parity-validated live deterministic export logic."
      }
    );
...
if (require.main === module) {
  run().catch((error) => {
    console.error("NQ historical replay failed.");
