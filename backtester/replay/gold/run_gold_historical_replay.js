...
const {
  buildReplayOutput,
  parseLogicVersion
} = require("./gold_replay_core");

const REPLAY_VERSION = "gold_historical_replay_v1";
const SOURCE_WORKFLOW = "gold_historical_replay";
...
async function fetchExistingObservation(supabaseUrl, serviceRoleKey, sourceSnapshotId) {
  ...
  url.searchParams.set("agent_name", "eq.GOLD");
...
async function upsertObservation(supabaseUrl, serviceRoleKey, snapshot) {
  const sourceSnapshotId = snapshot.id || `GOLD|${snapshot.snapshot_date}`;
  ...
  const payload = {
    ...
    market_regime: {
      ...
      geopolitical_risk_flag: snapshot.geopolitical_risk_flag ?? null
    },
...
async function upsertPrediction(supabaseUrl, serviceRoleKey, observationId, verdictId, prediction) {
  const payload = {
    ...
    mapping_notes: "Historical GOLD replay writes canonical following-24hrs predictions from the parity-validated 24h replay core.",
  ...
  for (const snapshot of selectedSnapshots) {
    ...
    await upsertSingletonByObservation(
      ...
      observation.id,
      {
        ...
        prompt_version: null,
        weight_model_version: "gold_weighted_logic_live_export_v1",
        conviction_model_version: "gold_conviction_logic_live_export_v1",
        workflow_version_id: REPLAY_VERSION,
        repo_commit_sha: null,
        notes: "Historical GOLD replay generated inside /backtester from the parity-validated 24h replay core."
      }
    );
    ...
    const verdict = await upsertSingletonByObservation(
      ...
      observation.id,
      {
        observation_id: observation.id,
        agent_name: "GOLD",
        reasoning_summary: replayOutput.reason_24h,
    ...
    const prediction = replayOutput.timeframe_models["24h"];
    const predictionRow = await upsertPrediction(supabaseUrl, serviceRoleKey, observation.id, verdict.id, {
      ...
      verdict_strength: prediction.conviction_model.verdict_strength,
...
if (require.main === module) {
  run().catch((error) => {
    console.error("GOLD historical replay failed.");
