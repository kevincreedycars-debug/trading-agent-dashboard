...
const DEFAULT_FLAT_THRESHOLD = 0.30;
...
async function run() {
  ...
  for (const prediction of predictions) {
    ...
    const openPrice = marketValueFromSnapshot("XAUUSD", observation.market_snapshot)
      ?? marketValueFromSeries("XAUUSD", callDate, priceMap);
    ...
    const closePrice = closeDate ? marketValueFromSeries("XAUUSD", closeDate, priceMap) : null;
    ...
    const row = {
      ...
      calibration_notes: finalEvaluation.evaluable
        ? "XAUUSD flat threshold currently treated as provisional for GOLD historical evaluation pending sensitivity review."
...
if (require.main === module) {
  run().catch((error) => {
    console.error("GOLD prediction outcome evaluation run failed.");
