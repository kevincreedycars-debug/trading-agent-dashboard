...
async function run() {
  ...
  for (const prediction of predictions) {
    ...
    const openPrice = marketValueFromSnapshot("EURUSD", observation.market_snapshot)
      ?? marketValueFromSeries("EURUSD", callDate, priceMap);
    ...
    const closePrice = closeDate ? marketValueFromSeries("EURUSD", closeDate, priceMap) : null;
    ...
    const row = {
      ...
      calibration_notes: finalEvaluation.evaluable
        ? "EURUSD flat threshold currently treated as provisional for EUR historical evaluation pending sensitivity review."
...
if (require.main === module) {
  run().catch((error) => {
    console.error("EUR prediction outcome evaluation run failed.");
