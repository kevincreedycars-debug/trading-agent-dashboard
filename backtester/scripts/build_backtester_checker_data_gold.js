...
const DEFAULT_OUTPUT = path.resolve(__dirname, "../../data/backtester-checker-gold-24h-2024-2026.json");
const DEFAULT_CALL_TIME_ET = "09:30:00";
const PERCENT_TOLERANCE = 0.5;
const GOLD_24H_FLAT_THRESHOLD = 0.30;
const GOLD_BENCHMARK_KEY = "xauusd_spot";
...
async function loadSnapshots(supabaseUrl, serviceRoleKey, startDate, endDate) {
  const rows = await fetchAllRows(
    supabaseUrl,
    serviceRoleKey,
    "historical_gold_market_snapshots",
    (url) => {
  ...
  return rows.filter((row) => isValidTradingSessionDay(row.snapshot_date));
}
...
async function loadPriceSeries(supabaseUrl, serviceRoleKey, startDate, endDate) {
  return fetchAllRows(
    ...
    (url) => {
      ...
      url.searchParams.set("instrument_key", `in.(${GOLD_BENCHMARK_KEY})`);
...
function marketValueFromSeries(market, closeDate, priceMap) {
  if (market === "XAUUSD") return priceMap.get(`${GOLD_BENCHMARK_KEY}|${closeDate}`) ?? null;
  return null;
}
...
function buildRowComparison(prediction, snapshot, storedEvaluation, priceMap) {
  ...
  const openPrice = marketValueFromSnapshot("XAUUSD", snapshot) ?? toNumber(storedEvaluation.open_price);
  const closePrice = marketValueFromSeries("XAUUSD", closeDate, priceMap) ?? toNumber(storedEvaluation.close_price);
  const rerunEvaluation = evaluateSingleMarket({
    ...
    evaluatedMarket: "XAUUSD",
    openPrice,
    closePrice,
    evaluationVersion: "phase1_outcome_eval_v1",
    flatThresholdOverride: GOLD_24H_FLAT_THRESHOLD,
  ...
  const comparisons = [
    ...
    compareNumeric("Headline Confidence %", prediction.predicted_conviction, rerunConvictionModel.final_conviction ?? null),
  ...
  return {
    ...
    checker: {
      ...
      headline_confidence_pct: rerunConvictionModel.final_conviction ?? null,
...
async function run() {
  ...
  const priceEndDate = addDays(endDate, 7);
  ...
  const payload = {
    meta: {
      ...
      replay_logic_source: "backtester/replay/gold/gold_replay_core.js",
      evaluation_logic_source: "backtester/lib/outcome_evaluation.js",
      evaluation_market: "XAUUSD",
      benchmark_instrument_key: GOLD_BENCHMARK_KEY,
      flat_threshold_used: GOLD_24H_FLAT_THRESHOLD,
      tolerance_percentage_points: PERCENT_TOLERANCE,
      session_rule: "weekday_only"
    },
    summary,
...
if (require.main === module) {
  run().catch((error) => {
    console.error("GOLD backtester checker data build failed.");
