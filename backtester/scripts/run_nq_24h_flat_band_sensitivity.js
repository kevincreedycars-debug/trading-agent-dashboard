...
const DEFAULT_THRESHOLDS = [0.20, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.60, 0.75];

async function loadNqRows(supabaseUrl, serviceRoleKey, startDate, endDate) {
...
async function run() {
  ...
  const rows = await loadNqRows(supabaseUrl, serviceRoleKey, startDate, endDate);
...
if (require.main === module) {
  run().catch((error) => {
    console.error("NQ flat-band sensitivity run failed.");
