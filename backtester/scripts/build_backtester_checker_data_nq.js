...
const { buildReplayOutput, parseLogicVersion } = require("../replay/nq/nq_replay_core");

const DEFAULT_START = "2024-01-02";
const DEFAULT_END = "2026-04-30";
const DEFAULT_OUTPUT = path.resolve(__dirname, "../../data/backtester-checker-nq-24h-2024-2026.json");
const DEFAULT_CALL_TIME_ET = "09:30:00";
const PERCENT_TOLERANCE = 0.5;
const NQ_24H_FLAT_THRESHOLD = 0.40;
...
async function loadStoredEvaluations(supabaseUrl, serviceRoleKey, startDate, endDate) {
  const rows = await fetchAllRows(
    ...
    (url) => {
      ...
      url.searchParams.set("timeframe", "eq.following 24hrs");
      url.searchParams.set("evaluated_market", "eq.QQQ_NQ_PROXY");
      ...
      url.searchParams.set("asset_code", "eq.NQ");
...
async function loadSnapshots(supabaseUrl, serviceRoleKey, startDate, endDate) {
  const rows = await fetchAllRows(
    supabaseUrl,
    serviceRoleKey,
    "historical_nq_market_snapshots",
    (url) => {
...
function buildRowComparison(prediction, snapshot, storedEvaluation, priceMap) {
  ...
  const openPrice = marketValueFromSnapshot("QQQ_NQ_PROXY", snapshot) ?? toNumber(storedEvaluation.open_price);
  const closePrice = marketValueFromSeries("QQQ_NQ_PROXY", closeDate, priceMap) ?? toNumber(storedEvaluation.close_price);
  const rerunEvaluation = evaluateSingleMarket({
    ...
    flatThresholdOverride: NQ_24H_FLAT_THRESHOLD,
...
async function run() {
  ...
  const payload = {
    meta: {
      ...
      replay_logic_source: "backtester/replay/nq/nq_replay_core.js",
      evaluation_logic_source: "backtester/lib/outcome_evaluation.js",
      evaluation_market: "QQQ_NQ_PROXY",
      benchmark_instrument_key: NQ_BENCHMARK_KEY,
      flat_threshold_used: NQ_24H_FLAT_THRESHOLD,
...
if (require.main === module) {
  run().catch((error) => {
    console.error("NQ backtester checker data build failed.");
