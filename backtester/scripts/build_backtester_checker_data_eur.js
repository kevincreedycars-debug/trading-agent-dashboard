...
const DEFAULT_OUTPUT = path.resolve(__dirname, "../../data/backtester-checker-eur-24h-2024-2026.json");
const DEFAULT_CALL_TIME_ET = "09:30:00";
const PERCENT_TOLERANCE = 0.5;
const EURUSD_24H_FLAT_THRESHOLD = 0.15;
...
function loadStoredPredictions(supabaseUrl, serviceRoleKey, startDate, endDate) {
  return fetchAllRows(
    ...
    (url) => {
      ...
      url.searchParams.set("observation.agent_name", "eq.EUR");
      url.searchParams.set("observation.source_workflow", "eq.eur_historical_replay");
...
function loadStoredEvaluations(supabaseUrl, serviceRoleKey, startDate, endDate) {
  return fetchAllRows(
    ...
    (url) => {
      ...
      url.searchParams.set("timeframe", "eq.following 24hrs");
      url.searchParams.set("evaluated_market", "eq.EURUSD");
      ...
      url.searchParams.set("asset_code", "eq.EUR");
...
function loadSnapshots(supabaseUrl, serviceRoleKey, startDate, endDate) {
  return fetchAllRows(
    supabaseUrl,
    serviceRoleKey,
    "historical_eur_market_snapshots",
    (url) => {
...
function loadPriceSeries(supabaseUrl, serviceRoleKey, startDate, endDate) {
  return fetchAllRows(
    supabaseUrl,
    serviceRoleKey,
    ...
    (url) => {
      ...
      url.searchParams.set("instrument_key", "in.(eurusd)");
...
function marketValueFromSnapshot(market, marketSnapshot) {
  if (market === "EURUSD") return toNumber(marketSnapshot?.eurusd_price);
  return null;
}

function marketValueFromSeries(market, closeDate, priceMap) {
  if (market === "EURUSD") return priceMap.get(`eurusd|${closeDate}`) ?? null;
  return null;
}
...
function buildRowComparison(prediction, snapshot, storedEvaluation, priceMap) {
  ...
  const openPrice = marketValueFromSnapshot("EURUSD", snapshot) ?? toNumber(storedEvaluation.open_price);
  const closePrice = marketValueFromSeries("EURUSD", closeDate, priceMap) ?? toNumber(storedEvaluation.close_price);
  const rerunEvaluation = evaluateSingleMarket({
    ...
    evaluatedMarket: "EURUSD",
    openPrice,
    closePrice,
    evaluationVersion: "phase1_outcome_eval_v1",
    flatThresholdOverride: EURUSD_24H_FLAT_THRESHOLD,
...
async function run() {
  ...
  const payload = {
    meta: {
      ...
      replay_logic_source: "backtester/replay/eur/eur_replay_core.js",
...
if (require.main === module) {
  run().catch((error) => {
    console.error("EUR backtester checker data build failed.");
