create table if not exists historical_gold_market_snapshots (
  id uuid primary key default gen_random_uuid(),

  asset_code text not null default 'GOLD',
  ...
  geopolitical_risk_flag boolean,

  collector_version text not null,
  snapshot_schema_version text not null,
  reconstruction_logic_version text not null,
  logic_document text not null default 'agent_gold_direction.md',
  ...
  constraint historical_gold_market_snapshots_asset_code_check
    check (asset_code = 'GOLD'),

  constraint historical_gold_market_snapshots_snapshot_mode_check
    check (snapshot_mode in ('historical_reconstruction', 'forward')),

  constraint historical_gold_market_snapshots_fed_bias_check
    check (fed_bias is null or fed_bias in ('hawkish', 'dovish', 'neutral', 'unknown')),

  constraint historical_gold_market_snapshots_equities_regime_check
    check (equities_regime is null or equities_regime in ('risk_on', 'risk_off', 'neutral')),

  constraint historical_gold_market_snapshots_source_status_check
    check (source_status in ('collected', 'partial', 'missing', 'reconstructed')),

  constraint historical_gold_market_snapshots_latest_us_event_age_hours_check
    check (latest_us_event_age_hours is null or latest_us_event_age_hours >= 0),

  constraint historical_gold_market_snapshots_observation_unique
    unique (asset_code, observation_time)
);

create index if not exists idx_historical_gold_market_snapshots_observation_time
  on historical_gold_market_snapshots (observation_time desc);

create index if not exists idx_historical_gold_market_snapshots_snapshot_date
  on historical_gold_market_snapshots (snapshot_date desc);

create index if not exists idx_historical_gold_market_snapshots_reconstructable
  on historical_gold_market_snapshots (is_reconstructable_following_24hrs, snapshot_date desc);

create index if not exists idx_historical_gold_market_snapshots_logic_version
  on historical_gold_market_snapshots (logic_document_version, reconstruction_logic_version);
