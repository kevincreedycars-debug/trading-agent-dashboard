...
const SNAPSHOT_TABLE = "historical_nq_market_snapshots";
const LOGIC_DOCUMENT = "agent_nq_direction.md";
const COLLECTOR_VERSION = "nq_historical_snapshot_builder_v1";
const SNAPSHOT_SCHEMA_VERSION = "v1";
const RECONSTRUCTION_LOGIC_VERSION = "nq_historical_reconstruction_v1";
const NQ_BENCHMARK_KEY = "qqq_nq_proxy";
const BTC_BENCHMARK_KEY = "btc_usd_spot";
const GOLD_BENCHMARK_KEY = "gold_spot_usd";
...
function listCandidateDates(seriesCollection, startDate, endDate) {
  ...
  for (const seriesState of Object.values(seriesCollection)) {
    for (const entry of seriesState.ordered) {
      if (
        entry.observation_date >= startDate &&
        entry.observation_date <= endDate &&
        isValidTradingSessionDay(entry.observation_date)
      ) {
...
function buildSnapshotRow(snapshotDate, context) {
  ...
  const rawBtc = current(BTC_BENCHMARK_KEY);
  const rawGold = current(GOLD_BENCHMARK_KEY);
  ...
  const nqD1 = percentDelta(rawNq, previousValue(context.series[NQ_BENCHMARK_KEY], snapshotDate, 1));
  const nqD5 = percentDelta(rawNq, previousValue(context.series[NQ_BENCHMARK_KEY], snapshotDate, 5));
  const nqD20 = percentDelta(rawNq, previousValue(context.series[NQ_BENCHMARK_KEY], snapshotDate, 20));
  const btcD1 = percentDelta(rawBtc, previousValue(context.series[BTC_BENCHMARK_KEY], snapshotDate, 1));
  const btcD5 = percentDelta(rawBtc, previousValue(context.series[BTC_BENCHMARK_KEY], snapshotDate, 5));
  const btcD20 = percentDelta(rawBtc, previousValue(context.series[BTC_BENCHMARK_KEY], snapshotDate, 20));
  const goldD1 = percentDelta(rawGold, previousValue(context.series[GOLD_BENCHMARK_KEY], snapshotDate, 1));
  const goldD5 = percentDelta(rawGold, previousValue(context.series[GOLD_BENCHMARK_KEY], snapshotDate, 5));
  const goldD20 = percentDelta(rawGold, previousValue(context.series[GOLD_BENCHMARK_KEY], snapshotDate, 20));
  ...
  const reconstructable3d = reconstructable24h &&
    vixD5 !== null &&
    dxyD5 !== null &&
    us10yD5 !== null &&
    us10yRealD5 !== null &&
  ...
  return {
    ...
    reconstruction_notes: "Daily observation-date reconstruction for the current NQ production logic using the live QQQ proxy benchmark convention.",
    ...
    history_rows_used: {
      ...
      [NQ_BENCHMARK_KEY]: context.series[NQ_BENCHMARK_KEY].byDate.has(snapshotDate) ? 1 : 0,
      [BTC_BENCHMARK_KEY]: context.series[BTC_BENCHMARK_KEY].byDate.has(snapshotDate) ? 1 : 0,
      [GOLD_BENCHMARK_KEY]: context.series[GOLD_BENCHMARK_KEY].byDate.has(snapshotDate) ? 1 : 0
    },
    is_reconstructable_following_24hrs: reconstructable24h,
    is_reconstructable_3d_from_call: reconstructable3d,
    is_reconstructable_current_week: reconstructable3d,
    is_reconstructable_next_week: reconstructable3d,
    is_reconstructable_current_month: reconstructable3d,
    raw_event_payload: latestEvent || {},
    raw_market_payload: {
      nq_benchmark_source: NQ_BENCHMARK_KEY,
      btc_benchmark_source: BTC_BENCHMARK_KEY,
      gold_benchmark_source: GOLD_BENCHMARK_KEY
    }
  };
}
...
async function loadMacroSeries(supabaseUrl, serviceRoleKey, startDate, endDate) {
  ...
  const keys = ["vix_level", "dxy_level", "us_10y_yield", "us_10y_real_yield"];
...
async function loadPriceSeries(supabaseUrl, serviceRoleKey, startDate, endDate) {
  const fetchStartDate = shiftDateLiteral(startDate, -60);
  const keys = [NQ_BENCHMARK_KEY, BTC_BENCHMARK_KEY, GOLD_BENCHMARK_KEY];
  ...
  console.log(JSON.stringify({
    ...
    sample_snapshot: rows[0]
      ? {
          ...
          nq_price: rows[0].nq_price,
          nq_d1_pct: rows[0].nq_d1_pct,
...
if (require.main === module) {
  run().catch((error) => {
    console.error("NQ historical snapshot build failed.");
