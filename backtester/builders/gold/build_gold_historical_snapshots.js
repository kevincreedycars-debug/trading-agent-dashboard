...
const SNAPSHOT_TABLE = "historical_gold_market_snapshots";
const LOGIC_DOCUMENT = "agent_gold_direction.md";
const COLLECTOR_VERSION = "gold_historical_snapshot_builder_v1";
const SNAPSHOT_SCHEMA_VERSION = "v1";
const RECONSTRUCTION_LOGIC_VERSION = "gold_historical_reconstruction_v1";
...
function buildSnapshotRow(snapshotDate, context) {
  ...
  return {
    ...
    reconstruction_notes: "Daily observation-date reconstruction for the current GOLD production logic.",
    ...
    raw_market_payload: {
      qqq_proxy_for_growth_regime: true,
...
async function loadPriceSeries(supabaseUrl, serviceRoleKey, startDate, endDate) {
  const fetchStartDate = shiftDateLiteral(startDate, -60);
  const keys = [GOLD_BENCHMARK_KEY, "qqq_nq_proxy"];
  ...
  console.log(JSON.stringify({
    ...
    sample_snapshot: rows[0]
      ? {
          ...
          gold_price: rows[0].gold_price,
          gold_d1_pct: rows[0].gold_d1_pct,
...
if (require.main === module) {
  run().catch((error) => {
    console.error("GOLD historical snapshot build failed.");
